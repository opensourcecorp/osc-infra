...
add_pgpass_file_for_replica:
  ...
  - replace: true
  - contents: |
      datastore.service.consul:{{ pillar['datastore_postgres_port'] }}:replication:replica:{{ pillar['datastore_postgres_replica_password'] }}
...
run_pg_basebackup:
  ...
  - name: |
      pg_basebackup \
        -h datastore.service.consul \
        -p {{ pillar['datastore_postgres_port'] }} \
        -U replica \
        --no-password \
